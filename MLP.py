...
class MLP_Tensorflow():
	...
	def prepare_model(self):
		...
		for hidden in range(len(self.hidden_layers_units)):
			...
			layers_hidden = tf.layers.dense(layers[-1], hidden, activation=tf.nn.relu, kernel_initializer=initializer,
		...
		with tf.Session() as sess:
			...
			for epoch in range(self.epochs):
				...
				for batch in range(len(self.data.X_train) // self.batch_size):
					...
					c, _, acc = sess.run([self.cost, self.optimizer, self.accuracy],
										 feed_dict={self.X: batch_x, self.Y: batch_y})
					epoch_loss += c
				
				self.cost_history = np.append(self.cost_history, epoch_loss)
				acc, p = sess.run([self.accuracy, tf.round(self.predicted)],
								  feed_dict={self.X: self.data.X_validation, self.Y: self.data.Y_validation})
				print('Epoch: ', epoch, 'Loss: ', epoch_loss, 'Validation Accuracy:', acc)
	
	def test_model(self):
		acc, p = self.sess.run([self.accuracy, tf.round(self.predicted)], feed_dict={self.X: self.data.X_test,
